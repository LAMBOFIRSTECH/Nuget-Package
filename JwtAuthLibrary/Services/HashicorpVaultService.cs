...
{
    private readonly string hashiCorpHttpClient = configuration["HashiCorp:HttpClient:VaultAddress"]!;
    private async Task<string> GetJwtTokenFromVaultUsingAppRole()
    {
    ...
    {
        string vautlAppRoleToken = await GetJwtTokenFromVaultUsingAppRole();
        var vaultClient = new VaultClient(new VaultClientSettings($"{hashiCorpHttpClient}", new TokenAuthMethodInfo(vautlAppRoleToken)));
        try
        {
            var secret = await vaultClient.V1.Secrets.KeyValue.V2.ReadSecretAsync(configuration["HashiCorp:JwtPublicKeyPath"]);
            if (secret == null)
            {
    ...
    public async Task<string> GetClientCertificatePassword()
    {
        string vautlAppRoleToken = await GetJwtTokenFromVaultUsingAppRole();
        var vaultClient = new VaultClient(new VaultClientSettings($"{hashiCorpHttpClient}", new TokenAuthMethodInfo(vautlAppRoleToken)));
        var secret = await vaultClient.V1.Secrets.KeyValue.V2.ReadSecretAsync(configuration["HashiCorp:CertPath"]);
        if (secret == null)
        ...
        if (!secretData.TryGetValue("certpass", out object? value))
            throw new VaultConfigurationException(404, "Error", "❌ Key 'password' not found for Client certificate.");
